...
namespace perception {
    ...
    public:
        ...
        double changed_min_x_;
        double changed_min_y_;
        double changed_max_x_;
        double changed_max_y_;
        ...
        std::vector<Point2D> getShortestPathByCV(const Point2D &goal, const Point2D &start) const;

        std::vector<Point2D> optimalToStraight(std::vector<Point2D> &path) const;

        std::vector<Point2D> optimalToStraightByCV(std::vector<Point2D> &path) const;
