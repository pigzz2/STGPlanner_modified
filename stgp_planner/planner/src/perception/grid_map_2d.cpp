...
namespace perception {
    ...
    std::vector<Point2D> GridMap2D::getShortestPath(const Point2D &goal, const Point2D &start) const {
        ...
        if (isInMapRange2D(start) && isInMapRange2D(goal)) {
            ...
            std::vector<double> dist(grid_x_num_ * grid_y_num_, INFINITY);
            std::vector<double> estimation(grid_x_num_ * grid_y_num_, INFINITY);
            const int INF = 0x3f3f3f3f;
            std::vector<int> backpointers(grid_x_num_ * grid_y_num_, INF);
