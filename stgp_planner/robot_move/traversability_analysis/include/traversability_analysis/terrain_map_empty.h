...
namespace traversability_analysis {
    ...
    public:
        ...
        pcl::PointCloud<pcl::PointXYZ> voxelized_local_cloud_;
        vector<vector<pcl::PointCloud<pcl::PointXYZ>>> grids_cloud_;
        std::vector<std::vector<float>> grids_min_elevation_;
        std::vector<std::vector<pcl::PointXYZ>> grids_bottom_point_;
